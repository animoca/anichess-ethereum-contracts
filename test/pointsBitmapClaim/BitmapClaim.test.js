...
describe('BitmapClaim', function () {
  ...
  describe('setBitValue(uint256 bitPosition, uint256 value)', function () {
    it('Reverts with {NotContractOwner} if not called by owner', async function () {
      const bitPosition = 1;
      const value = 100;

      await expect(this.contract.connect(other).setBitValue(bitPosition, value))
    ...
    it('Reverts with {BitPositionTooBig} if bitPosition is larger than maxBitCount', async function () {
      ...
      await expect(this.contract.connect(deployer).setBitValue(bitPosition, value))
    ...
    context('when successful', function () {
      it('sets value for bitPosition', async function () {
        ...
        await this.contract.connect(deployer).setBitValue(bitPosition, value);

        expect(await this.contract.bitPositionValueMap(bitPosition)).to.equal(value);
      });
      it('sets maxBitCount increases by 1', async function () {
        const bitPosition = 0;
        const value = 100;
  ...
  describe('claim(address recipient, uint256 claimBits, bytes calldata validationData)', function () {
    ...
    it('Reverts with {AlreadyClaimed} if one of the the given claimBits has been claimed', async function () {
      await this.contract.connect(deployer).setBitValue(0, 100);
      await this.contract.connect(deployer).setBitValue(1, 200);
