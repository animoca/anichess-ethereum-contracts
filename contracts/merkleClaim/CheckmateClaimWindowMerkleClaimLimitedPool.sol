...
contract CheckmateClaimWindowMerkleClaimLimitedPool is CheckmateClaimWindowMerkleClaim {
    ...
    constructor(address checkmateToken_, address nft_, address stakingPool_, address payoutWallet_, IForwarderRegistry forwarderRegistry_) CheckmateClaimWindowMerkleClaim(checkmateToken_, stakingPool_, payoutWallet_, forwarderRegistry_) {
        if (nft_ == address(0)) {
            revert InvalidNFT();
        }
    ...
    function _canClaim(ClaimWindow storage claimWindow, address recipient, uint256 /* amount */, bytes calldata data, bytes32 leaf) internal view override returns (bool) {
        if (claimWindow.merkleRoot == bytes32(0) || block.timestamp < claimWindow.startTime || block.timestamp > claimWindow.endTime || claimed[leaf]) {
            return false;
        }

        uint256[] memory nftTokenIds = abi.decode(data, (uint256[]));
        uint256 len = nftTokenIds.length;
        for (uint256 i; i < len; ++i) {
            if (NFT.ownerOf(nftTokenIds[i]) != recipient) {
                return false;
            }
        }
    ...
    function _calculateLeafHash(bytes32 epochId, address recipient, uint256 amount, bytes calldata data) internal pure override returns (bytes32) {
        return keccak256(abi.encodePacked(epochId, recipient, amount, data));
    }
}
